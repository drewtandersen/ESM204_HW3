...
The benefit to consumers is $`r con_surplus` 
The benefit to producers is $`r con_surplus` 
The environmental cost is $`env_cost`
...
The consumer benefit for the "High" group is `r high_ben`
The consumer benefit for the "Low" group is `r low_ben`
...
```
(a) Quantity (kwh) = `r new_q` kwh
(b) Price (cents) = `r new_p` cents
(c) "High" Group Welfare = $`r high_ben_tax`
(d) "Low" Group Welfare = $`r low_ben_tax`
(e) Power Supplier Surplus = $`r prod_surplus_tax`
(f) Environmental Cost = $`r env_cost_tax`
(g) Total tax revenue = $`r tax_rev`
...
solar_agg_slope <- 1/(low_lm_inv$coefficients[2] + solar_lm$coefficients[2])
